...
author: "Yunwei Zhang"
date: “`r paste0('Initiated on 20240821, compiled on ', format(Sys.time(), '%Y %b %d'))`”
output:
...
This file is used to wrap up findings for paper. for real data amalysis on the ov cancer data.
...
```{r}
load("uvm_real_result0612.RData")
...
# uvm data with outliers

insert outliers: randomly select observations (5%) to have long survival time
```{r}
hist(current_data2$time)
time=current_data2$time
sample_size <- ceiling(length(time) * 0.05)
set.seed(20240813)
sampled_indices <- sample(seq_along(time), sample_size)
time[sampled_indices] <- time[sampled_indices] + 1000
current_data3=current_data2
current_data3$time=time
hist(current_data3$time)
```


```{r eval=FALSE}
current_data2=current_data3
lasso_result=result_cal(current_data2,model_name="lasso")
enet_result=result_cal(current_data2,model_name="enet")
alasso_result=result_cal(current_data2,model_name="alasso")
aenet_result=result_cal(current_data2,model_name="aenet")
...
scad_result=NA
pawph_result=NA
save(lasso_result,enet_result,alasso_result,aenet_result,mcp_result,scad_result,mnet_result,snet_result,pawph_result,pawphmcp_result,pawphscad_result,sis_result,file="uvm_real_resultoutliers0813.RData")
```


```{r}
load("uvm_real_resultoutliers0813.RData")
sis_result1=sis_result[[1]]
ids=gsub("X", "", names(sis_result1))
ids2=as.numeric(ids)+21
names(sis_result1)=colnames(current_data2)[ids2]
...
df[,1:(ncol(df)-1)]=round(df[,1:(ncol(df)-1)],1)
df<-df%>%
...
ggheatmap <-ggplot(gg_heatmap_data, aes(variables, method, fill= value)) +
 geom_tile(color = "grey")+scale_fill_gradientn(limits = c(-4,4),colours = c("#2166AC", "#67A9CF" ,"#D1E5F0", "#FFFFFF","#FDDBC7", "#EF8A62", "#B2182B"))+ theme(aspect.ratio = 1, text = element_text(size = 15), legend.position = "bottom") + labs(y= 'Method', x = "Variables", fill = 'Estimated params')+theme_bw()+theme(axis.text.x = element_text(color = "black", size = 15,angle = 90),axis.text.y = element_text(color = "black", size = 16))+
  geom_text(aes(label = round(value,1)), color = "black", size = 4)
ggheatmap
#ggsave(ggheatmap,filename="uvm_heatmapoutliers0813.pdf",width = 17,height = 8)

```


insert outliers: randomly select observations (20%) to have long survival time
```{r}
current_data=read.csv("UVM.csv")
current_data2=current_data
colnames(current_data2)[(dim(current_data2)[2]-2):dim(current_data2)[2]]=c("control_var","status","time")
...
hist(current_data2$time)
time=current_data2$time
sample_size <- ceiling(length(time) * 0.2)
set.seed(20240813)
sampled_indices <- sample(seq_along(time), sample_size)
time[sampled_indices] <- time[sampled_indices] + 1000
current_data3=current_data2
...
pawph_result=NA
mnet_result=NA
save(lasso_result,enet_result,alasso_result,aenet_result,mcp_result,scad_result,mnet_result,snet_result,pawph_result,pawphmcp_result,pawphscad_result,sis_result,file="uvm_real_resultoutliers20813.RData")
```


```{r}
load("uvm_real_resultoutliers20813.RData")
sis_result1=sis_result[[1]]
ids=gsub("X", "", names(sis_result1))
ids2=as.numeric(ids)+21
names(sis_result1)=colnames(current_data2)[ids2]
...
my_list=list(lasso_result1,enet_result1,snet_result1,pawphmcp_result[[1]],pawphscad_result[[1]],sis_result1)
...
# df$method=factor(c("lasso","enet","mnet","snet","pawph_mcp","pawph_scad","sis"),levels = c("lasso","enet","mnet","snet","pawph_mcp","pawph_scad","sis"))
df$method=factor(c("lasso","enet","snet","pawph_mcp","pawph_scad","sis"),levels = c("sis","snet","enet","lasso","pawph_mcp","pawph_scad"))
...
ggheatmap <-ggplot(gg_heatmap_data, aes(variables, method, fill= value)) +
 geom_tile(color = "grey")+scale_fill_gradientn(limits = c(-4,4),colours = c("#2166AC", "#67A9CF" ,"#D1E5F0", "#FFFFFF","#FDDBC7", "#EF8A62", "#B2182B"))+ theme(aspect.ratio = 1, text = element_text(size = 15), legend.position = "bottom") + labs(y= 'Method', x = "Variables", fill = 'Estimated params')+theme_bw()+theme(axis.text.x = element_text(color = "black", size = 15,angle = 90),axis.text.y = element_text(color = "black", size = 16))+
  geom_text(aes(label = round(value,1)), color = "black", size = 4)
ggheatmap
#ggsave(ggheatmap,filename="uvm_heatmapoutliers20813.pdf",width = 17,height = 8)
