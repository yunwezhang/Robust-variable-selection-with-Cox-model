...
prcox <- function(y, X, penalty.factor=c(rep(0,p),  rep(1,n)), lambda.min=0.05, lambda, ...){
  ...
  # lasso fit
  if(missing(lambda)){
    ncvsurv(X.aug, y, penalty="SCAD", lambda.min = lambda.min, 
            penalty.factor = penalty.factor, returnX = FALSE, ...)
  }else{
    ncvsurv(X.aug, y, penalty="SCAD",  penalty.factor = penalty.factor,
...
cv.prcox.adap <- function(y, X, seed, alpha=0.01, penalty.factor=rep(1, p+n), lambda.min=0.05,
                          pout.max=NULL, ...){
  p <- ncol(X)
  n <- nrow(X)
...
prcoxreg <- function(y, X, seed, alpha=0.01, cond.CI=FALSE,pout.max=0.2, permu.weight=rep(1, n), ...){
  p <- ncol(X)
  n <- nrow(X)
  delta <- y[,2]
  ...
  if(sum(res$betaHat!=0)>0){
    fit <- ncvsurv(as.matrix(X[,which(res$betaHat!=0)]), y, penalty="SCAD",intercept =res$gammaHat, lambda=0, returnX = FALSE,permu.weight = permu.weight)
    betaHat_re <- rep(0,p)
