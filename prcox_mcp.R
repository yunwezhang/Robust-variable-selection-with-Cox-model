...
prcox <- function(y, X, penalty.factor=c(rep(0,p),  rep(1,n)), lambda.min=0.05, lambda, ...){
  ...
  # lasso fit
  if(missing(lambda)){
    ncvsurv(X.aug, y, penalty="MCP", lambda.min = lambda.min, 
            penalty.factor = penalty.factor, returnX = FALSE, ...)
  }else{
    ncvsurv(X.aug, y, penalty="MCP",  penalty.factor = penalty.factor,
...
prcoxreg <- function(y, X, seed, alpha=1, cond.CI=FALSE,pout.max=0.2, permu.weight=rep(1, n), ...){
  ...
  if(sum(res$betaHat!=0)>0){
    fit <- ncvsurv(as.matrix(X[,which(res$betaHat!=0)]), y, penalty="MCP",intercept =res$gammaHat, lambda=0, returnX = FALSE,permu.weight = permu.weight)
    betaHat_re <- rep(0,p)
